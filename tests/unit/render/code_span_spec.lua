...
describe("render.code_span", function()
  ...
  -- row 39: "Inline `code` here."
  it("highlights the span including its backticks", function()
    local hl = h.find(by_row, 39, function(m)
      return h.has_hl("PreviewCodeInline")(m) and m.opts.conceal == nil
    end)
    assert.are.same({ { row = 39, col = 7, opts = { end_row = 39, end_col = 13, hl_group = "PreviewCodeInline" } } }, hl)
  end)

  it("turns each backtick into one chip-colored space", function()
    local conceals = h.find(by_row, 39, h.is_conceal)
    table.sort(conceals, function(a, b)
      return a.col < b.col
    end)
    assert.are.same({
      { row = 39, col = 7, opts = { end_col = 8, conceal = " ", hl_group = "PreviewCodeInline" } },
      { row = 39, col = 12, opts = { end_col = 13, conceal = " ", hl_group = "PreviewCodeInline" } },
    }, conceals)
  end)
end)
