...
describe("render.heading", function()
  ...
  local function is_space_above(m)
    return m.opts.virt_lines ~= nil and m.opts.virt_lines_above == true
  end
  ...
  it("adds the level's virtual blank lines above each heading by default", function()
    for level = 1, 6 do
      local row = ROWS[level]
      local hl = "PreviewH" .. level
      local count = ({ 2, 1, 0, 0, 0, 0 })[level]
      local marks = h.find(by_row, row, is_space_above)
      if count == 0 then
        assert.equals(0, #marks, "row " .. row)
      else
        local blank = {}
        for i = 1, count do blank[i] = { { "", hl } } end
        assert.are.same({ { row = row, col = 0, opts = { virt_lines = blank, virt_lines_above = true } } }, marks, "row " .. row)
      end
    end
  end)

  it("adds no virtual line when a level's space_above is 0 and three when it is 3", function()
    local none = h.marks_for(buf, h.with_config(ctx, { heading = { space_above = { 0, 0, 0, 0, 0, 0 } } }))
    assert.equals(0, #h.find(none, 0, is_space_above))

    local three = h.marks_for(buf, h.with_config(ctx, { heading = { space_above = { 3, 0, 0, 0, 0, 0 } } }))
    local marks = h.find(three, 0, is_space_above)
    assert.equals(1, #marks)
    assert.equals(3, #marks[1].opts.virt_lines)
  end)
