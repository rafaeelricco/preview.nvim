...
describe("preview.config", function()
  ...
  it("resolves the reading-view defaults", function()
    ...
    assert.are.same({ 2, 1, 0, 0, 0, 0 }, cfg.render.heading.space_above)
  ...
  it("rejects a heading.space_above entry below 0 with the dotted key", function()
    local cfg, err = config.resolve({ render = { heading = { space_above = { 1, 1, 1, 1, 1, -1 } } } })
    assert.is_nil(cfg)
    assert.is_true(contains(err, "render.heading.space_above"))
  end)
